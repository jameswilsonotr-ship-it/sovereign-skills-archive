...
This document is analysis and a recommendation record. It does not create
forks, vendor dependencies, or authorize a live skill-tree migration.
...
**Analysis source:** a local shallow clone used while preparing the source
branch; the clone is not committed to this repository.
...
**Local skill:** `skill_tree/skills/icm-architect/`
...
The mainline promotion contains the analysis needed for review. Any
additional long-form dumps or upstream clones from the source branch remain
local-only and are not required to interpret this document.
