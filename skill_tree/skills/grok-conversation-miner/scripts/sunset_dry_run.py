...
from harvest_hooks import log_lanes
...
def main(argv=None) -> int:
    ...
    p.add_argument("--log", type=Path, help="append-only EXPORT_LOG.jsonl (020)")
    p.add_argument("--conversation-key", default="dry-run")
    ...
    rid = run_id("DRY")
    if args.log:
        log_lanes(args.log, run_id=rid, verb="dry-run", conversation_key=args.conversation_key, lanes=table, dry_run=True)
    payload = {"flag": args.flag, "lanes": table, "run_id": rid, **stamps()}
    text = json.dumps(payload, indent=2, sort_keys=True)
    if args.out:
        args.out.parent.mkdir(parents=True, exist_ok=True)
        args.out.write_text(text, encoding="utf-8")
    log_lanes(stamps().get('date_ny','dry') + '-dry', 'sunset-dry-run', args.key or 'fixture', table)
    print(text)
    return 0
