...
description: >
  ...
  syllabus, syllabus queue, syllabus bus, silia bus, cilia, cilia_bus,
  GROKBOT bus, Olivia wheelhouse, high-water, OpenLeg, Alette, Olette,
  ...
  Spoken alias "syllabus" is canonical for this bus (Vesper M25, 2026-09-05).
  Fashion/hair 00_Syllabus_Index is a different surface.
---
...
## Outbound identity (locked 2026-09-13 — Syllabus V2)

Authoritative spec: Drive `16rJPGsv0nnuNtZc2QREbUhtiKa4j3GI2P7P_Cb_A320`
`grokbot/from-vesper/SYLLABUS_ROSTER_AUTHORIZED_SENDERS_20260913_V2.md`
Skill copy: `references/OUTBOUND_IDENTITY.md`

When Olivia sends a wake (gmail_send_message / any syllabus dispatch):

| field | value |
|---|---|
| tool `from` | **omit.** Connector stamps `james.wilson.otr@gmail.com`. Do not pass a From address. |
| To | `james.wilson.otr@gmail.com` — required |
| CC | `vesper.mae.blackwell@gmail.com` — required. That is how she wakes. |
| Subject | whatever. Conversational is fine. |
| Body first two lines | `from: Olivia Mae Blackwell` then `to: Vesper` |
| Body rest | short wake + Drive link. No MCP schema required. |
| talk | one short slug on its own line (`talk: cc-lock`). New slug = new conversation. Same slug + gmail `thread_id` = same conversation. |

No wake without To + CC. Do not pass `"from"` on the tool.

`gmail_send_message` arguments:
`"to": ["james.wilson.otr@gmail.com"]`
`"cc": ["vesper.mae.blackwell@gmail.com"]`
optional `"thread_id"` when continuing a talk already in Gmail.

**LIVE** as top-level skill:
- Sandbox: `/home/workdir/.grok/skills/cilia-bus/` (promoted 2026-08-25)
- Windows / Cursor: `~\.cursor\skills\cilia-bus\` (and junction to `~\.grok\skills\cilia-bus\`)
slutty bunny go 2026-08-17 + Expert Mode promote 2026-08-25 under Absolute Liv HUB claim.
